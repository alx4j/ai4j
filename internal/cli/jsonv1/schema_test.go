...
func TestAllCommandResponsesValidateAgainstPublishedSchemas(t *testing.T) {
	...
	for _, fixture := range commandFixtures(t) {
		...
		t.Run(fixture.command.String(), func(t *testing.T) {
			t.Parallel()
			encoded, err := jsonv1.Marshal(fixture.response)
			if err != nil {
	...
	encoded, err := jsonv1.Marshal(usage)
	if err != nil {
...
func TestListResponseValidatesAgainstPublishedSchema(t *testing.T) {
	...
	encoded, err := jsonv1.Marshal(response)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSourceProvenanceGoldenKeepsTypedIdentitiesDistinct(t *testing.T) {
	...
	encoded, err := jsonv1.Marshal(commandFixtures(t)[0].response)
	if err != nil {
		t.Fatalf("Marshal() error = %v", err)
	}
	var wire jsonv1.Envelope[jsonv1.ValidateData]
...
func TestEveryFailureFamilyValidatesWithUnavailableData(t *testing.T) {
	...
	for _, failure := range failures {
		...
		t.Run(failure.String(), func(t *testing.T) {
			...
			encoded, marshalErr := jsonv1.Marshal(response)
			if marshalErr != nil {
	...
	encoded, err := jsonv1.Marshal(cancelled)
	if err != nil {
...
func TestMutationRecoverySemanticsValidate(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			validateJSON(t, schema, encoded)
			var wire jsonv1.Envelope[jsonv1.MutationData]
			if err := json.Unmarshal(encoded, &wire); err != nil {
				t.Fatalf("Unmarshal() error = %v", err)
			}
...
func TestSchemaCompatibilityAndClosedEnums(t *testing.T) {
	...
	statusEncoded, _ := jsonv1.Marshal(commandFixtures(t)[5].response)
	...
	installEncoded, _ := jsonv1.Marshal(commandFixtures(t)[2].response)
	...
	versionEncoded, _ := jsonv1.Marshal(commandFixtures(t)[8].response)
	...
	usageEncoded, _ := jsonv1.Marshal(usageResponse(t))
...
func TestRoundTripNullEmptyAndDeterminism(t *testing.T) {
	...
	first, err := jsonv1.Marshal(version)
	if err != nil {
		t.Fatalf("Marshal() error = %v", err)
	}
	second, err := jsonv1.Marshal(version)
	if err != nil {
	...
	var decoded jsonv1.Envelope[jsonv1.VersionData]
	...
	usage, err := jsonv1.Marshal(usageResponse(t))
	if err != nil {
		t.Fatalf("Marshal(usage) error = %v", err)
	}
	want := `{"schemaVersion":1,"command":null,"status":"error","changed":false,"operationId":null,"exitCode":2,"data":null,"warnings":[],"errors":[{"code":"invalid_cli_usage","message":"command line does not match the MVP grammar","context":[{"field":"issue","value":"missing_command"}]}]}`
...
func TestNativeVersionObservationSemantics(t *testing.T) {
	...
	for _, native := range valid {
		...
		var wire jsonv1.Envelope[jsonv1.StatusData]
		if err := json.Unmarshal(encoded, &wire); err != nil {
	...
	encoded, err := jsonv1.Marshal(unobservableResponse)
	if err != nil {
...
func TestStatusInstallationDispositionSemantics(t *testing.T) {
	...
	absentEncoded, _ := jsonv1.Marshal(absentResponse)
	statusSchema := compileSchemas(t)["status.json"]
	...
	presentEncoded, _ := jsonv1.Marshal(presentResponse)
	...
	recoveryEncoded, _ := jsonv1.Marshal(recoveryResponse)
...
func TestWireDTOsContainNoMapInterfaceOrRawJSONFields(t *testing.T) {
	...
	types := []reflect.Type{
		reflect.TypeOf(jsonv1.Envelope[jsonv1.ValidateData]{}), reflect.TypeOf(jsonv1.Diagnostic{}),
		reflect.TypeOf(jsonv1.ValidateData{}), reflect.TypeOf(jsonv1.PlanData{}), reflect.TypeOf(jsonv1.MutationData{}),
		reflect.TypeOf(jsonv1.StatusData{}), reflect.TypeOf(jsonv1.DoctorData{}), reflect.TypeOf(jsonv1.VersionData{}),
	}
