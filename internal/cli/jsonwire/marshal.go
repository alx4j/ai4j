...
// Marshal projects a validated neutral response to the stable JSON wire format.
func Marshal(response cli.Response) ([]byte, error) {
	...
	case cli.UsageData:
		...
		wire.Errors = []Diagnostic{{Code: "invalid_cli_usage", Message: "command line does not match the CLI grammar", Context: context}}
...
func listData(value cli.ListData) ListData {
	...
	return ListData{Installations: items}
}
