...
type HistoryEntry struct {
	...
	NativeArtifactBefore []byte  `json:"nativeArtifactBefore,omitempty"`
	NativeArtifactAfter  []byte  `json:"nativeArtifactAfter,omitempty"`
}
...
func (e HistoryEntry) Validate() error {
	...
	if e.Restorable && (e.Before != nil && e.Before.Lifecycle == "active" && len(e.NativeArtifactBefore) == 0 || e.After != nil && e.After.Lifecycle == "active" && len(e.NativeArtifactAfter) == 0) {
		return ErrMalformedHistory
	}
	if len(e.CatalogBefore)+len(e.RulesBefore)+len(e.CatalogAfter)+len(e.RulesAfter)+len(e.NativeArtifactBefore)+len(e.NativeArtifactAfter) > maximumHistoryBytes {
		return ErrMalformedHistory
	}
	return nil
}
...
func (s Store) CommitHistory(entry HistoryEntry) error {
	...
	expected, err := marshalHistory(staged)
	if err != nil {
		return err
	}
	...
	return s.writeHistory(entry, false, expected)
}
...
func (s Store) LoadHistory(installationID string) ([]HistoryEntry, error) {
	...
	for _, entry := range entries {
		...
		if value.InstallationID != installationID || !value.Committed {
			continue
		}
