...
func TestLifecycleClaudeUserLifecycleRetainsRollbackHistoryAndTombstone(t *testing.T) {
	...
	sync := parseRequest[cli.SyncRequest](t, "sync", installationID, "--asset", "minimal", "--yes")
...
func TestLifecycleDryRunsReturnPlansWithoutLockingPromptingOrMutation(t *testing.T) {
	...
	tests := []struct {
		...
		{cli.CommandSync, func(io CommandIO) (cli.Response, error) {
			request := parseRequest[cli.SyncRequest](t, "sync", record.InstallationID, "--asset", "minimal", "--dry-run")
...
func TestLifecycleWindowsClaudeUserJourneyUsesWindowsStateAndHost(t *testing.T) {
	...
	sync := parseRequest[cli.SyncRequest](t, "sync", record.InstallationID, "--asset", "minimal", "--yes")
...
func TestLifecycleIndependentInstallationsAndOwnedConflictPolicies(t *testing.T) {
	harness := newLifecycleHarness(t)
	first := parseRequest[cli.InstallRequest](t, "install", "--target", "claude", "--scope", "user", "--all", "--yes")
	...
	second := parseRequest[cli.InstallRequest](t, "install", "--target", "claude", "--scope", "user", "--asset", "other", "--yes")
	...
	replace := parseRequest[cli.SyncRequest](t, "sync", firstRecord.InstallationID, "--all", "--conflict-policy", "replace-owned", "--yes")
...
func TestLifecycleLocalDevelopmentInstallUpdateAndSyncUseImmutableBackingBundle(t *testing.T) {
	...
	install := parseRequest[cli.InstallRequest](t, "install", "--source", checkout, "--target", "claude", "--scope", "user", "--all", "--expected-source-digest", harness.validator.localDigest, "--yes")
	response, err := harness.service.Install(context.Background(), install, CommandIO{})
	if err != nil || response.Result().ExitCode() != result.ExitSuccess {
	...
	sync := parseRequest[cli.SyncRequest](t, "sync", record.InstallationID, "--asset", "minimal", "--allow-dirty", "--expected-source-digest", harness.validator.localDigest, "--yes")
	response, err = harness.service.Sync(context.Background(), sync, CommandIO{})
	if err != nil || response.Result().ExitCode() != result.ExitSuccess {
...
func (v *lifecycleValidator) SelectLifecycle(_ context.Context, options cli.SourceOptions, all bool, assets, bundles []string) validation.LifecycleSelection {
	commit := strings.Repeat("a", 40)
	...
	return validation.LifecycleSelection{Source: v.source(options, commit), ToolkitID: toolkitID, ToolkitVersion: "1.0.0", PackageID: packageID, PackagePath: packagePath, Content: []cli.ContentItem{item}, Rules: rules, RulesChecksum: digest, NativeArtifact: testLifecycleArtifact(), Resolved: resolved}
}
