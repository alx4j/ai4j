...
func TestCommandsFailBeforeBoundedWritesWhenCapacityIsInsufficient(t *testing.T) {
	...
	buildRequest, _ := cli.NewParser("darwin").Parse([]string{"ai4j", "build", "--target", "codex", "--host", "darwin-arm64", "--output", output, "--all"})
	...
	initRequest, _ := cli.NewParser("darwin").Parse([]string{"ai4j", "init", "--target", "codex", "--output", initOutput})
