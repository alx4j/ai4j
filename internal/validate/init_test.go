...
func TestInitCreatesValidDeterministicTargetScaffold(t *testing.T) {
	...
	for _, name := range []string{"first", "second"} {
		...
		request, parseErr := cli.Parse([]string{"ai4j.exe", "init", "--target", "claude", "--target", "codex", "--output", output, "--examples"})
		if parseErr != nil {
			t.Fatal(parseErr)
		}
	...
	buildRequest, err := cli.Parse([]string{"ai4j", "build", "--target", "codex", "--host", "darwin-arm64", "--output", buildOutput, "--bundle", "examples"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestInitRefusesOccupiedOutputWithoutChangingIt(t *testing.T) {
	...
	request, _ := cli.Parse([]string{"ai4j.exe", "init", "--target", "codex", "--output", output})
	report := service.Init(context.Background(), request.(cli.InitRequest))
